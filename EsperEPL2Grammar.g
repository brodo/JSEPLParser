...
//----------------------------------------------------------------------------
expressionDecl : EXPRESSIONDECL classIdentifier? (array=LBRACK RBRACK)? expressionDialect? name=IDENT (LPAREN columnList? RPAREN)? expressionDef;
...
selectExpr : (INSERT insertIntoExpr)? 
		SELECT selectClause
		(FROM fromClause)?
		matchRecog?
		(WHERE whereClause)?
...
createVariableExpr : CREATE c=IDENT? VARIABLE classIdentifier (arr=LBRACK RBRACK)? n=IDENT (EQUALS expression)?;
...
createColumnListElement : classIdentifier (classIdentifier (b=LBRACK RBRACK)?) ;
...
streamFilterExpression : IDENT (DOT viewExpression (DOT viewExpression)*)?;
...
matchRecogPatternUnary : matchRecogPatternNested 
		| matchRecogPatternAtom;

matchRecogPatternNested : LPAREN matchRecogPatternAlteration RPAREN (s=STAR | s=PLUS | s=QUESTION)?;
		
matchRecogPatternAtom :	i=IDENT ((s=STAR | s=PLUS | s=QUESTION) (reluctant=QUESTION)? )?;
...
builtinFunc : SUM LPAREN (ALL | DISTINCT)? expression aggregationFilterExpr? RPAREN   	#builtin_sum
		| AVG LPAREN (ALL | DISTINCT)? expression aggregationFilterExpr? RPAREN		#builtin_avg
		| COUNT LPAREN
			(
				((a=ALL | d=DISTINCT)? expression)
			|
				(STAR) 
			)
			aggregationFilterExpr? RPAREN						#builtin_cnt
		| MEDIAN LPAREN (ALL | DISTINCT)? expression aggregationFilterExpr? RPAREN	#builtin_median
		| STDDEV LPAREN (ALL | DISTINCT)? expression aggregationFilterExpr? RPAREN	#builtin_stddev
		| AVEDEV LPAREN (ALL | DISTINCT)? expression aggregationFilterExpr? RPAREN	#builtin_avedev
		| firstLastAggregation								#builtin_firstlast
		| windowAggregation								#builtin_window
		...
		| CAST LPAREN expression (COMMA | AS) classIdentifier RPAREN chainedFunction?	#builtin_cast
...
firstLastAggregation : (FIRST | LAST) LPAREN (accessAggExpr (COMMA expression)?)? RPAREN chainedFunction?;

lastAggregation : LPAREN (accessAggExpr (COMMA expression)?)? RPAREN chainedFunction?;
	
windowAggregation : WINDOW LPAREN accessAggExpr? RPAREN chainedFunction?;

accessAggExpr : s=STAR
		| propertyStreamSelector
		| expression;

aggregationFilterExpr :	COMMA expression;
...
libFunctionWithClass : (classIdentifier DOT)? funcIdentTop (l=LPAREN libFunctionArgs? RPAREN)?;	
...
observerExpression : ns=IDENT COLON (nm=IDENT | a=AT) LPAREN expressionWithTimeList? RPAREN;
...
classIdentifierNonGreedy : i1=escapableStr (DOT i2=escapableStr)*?;
